...
Tags: widgets, everyblock, widget, news, chicago, illinois, media
Tested up to: 3.6.1
...
Add a Chicago Media Widget Powered by Everyblock to a post or page.  Will also allow you to add as a widget.
...
5. If you want to embed in a post, use the shortcode [display_chicago_media_widget].  This will add a 300x500 widget.  You can specify sizes by adding the attributes width and height (example: [display_chicago_media_widget width=400 height=600]).
...
== Screenshots ==
1. The media widget added to a page.
...
= How do I add the media widget to a post or page? =

After activating the plugin, add the shortcode [display_chicago_media_widget] to any page or post.  

= How do I change the width and height of the widget while it is embedded in a page or post? =

Add the shortcode attributes width and height.  Example: [display_chicago_media_widget width=400 height=600].

= How do I add the food inspections widget to a sidebar, header, or footer? =

After activating the plugin, go to Appearance -> Settings, and drag the "Chicago Media Widget" to the appropriate area. 
