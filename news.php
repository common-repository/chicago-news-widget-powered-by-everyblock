<?php
/*
Plugin Name: Chicago Media Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Media Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagomediaWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "news-articles";
	
	public function ChicagomediaWidget()
	{
		$widget_ops = array('classname' => 'ChicagomediaWidget', 'description' => 'Embeds a Chicago Media Widget' );
		$this->WP_Widget('ChicagomediaWidget', 'Chicago Media Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagomediaWidget");') );

function display_chicago_media_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_media_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagomediaWidget::$metro . "&schema=" . ChicagomediaWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_media(stripslashes($code));
}

function get_html_for_embed_chicago_media($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_media_widget', display_chicago_media_widget);
